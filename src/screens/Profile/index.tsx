    ...
    appointments: [
      {
        ...
        time: "13:00",
        name: "João Souza",
        description: "Orientação 1",
      },
      {
        id: "2",
        time: "14:00",
        name: "João Souza",
        description: "Orientação 2",
      },
    ],
  },
  {
...
export function Profile() {
  ...
  const handleRemoveAppointment = (appointmentId: string) => {
    const updatedSchedules = schedules
      .map((dateGroup) => ({
        ...dateGroup,
        appointments: dateGroup.appointments.filter(
  ...
  return (
    <Container>
      <Header>
        ...
        <Subtitle>Aqui você pode ver todos os Agendamentos</Subtitle>
      </Header>
        ...
        renderItem={({ item }) => (
          <ScheduleCard past={isPastDate(item.date)}>
            <DateBox past={isPastDate(item.date)}>
              <IconBox name="calendar" past={isPastDate(item.date)} />
              <DateText past={isPastDate(item.date)}>{item.date}</DateText>
            </DateBox>
